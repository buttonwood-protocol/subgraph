...
export class BondFactory extends ethereum.SmartContract {
  ...
  bonds(param0: Bytes): Address {
    let result = super.call("bonds", "bonds(bytes32):(address)", [ethereum.Value.fromFixedBytes(param0)]);

    return result[0].toAddress();
  }

  try_bonds(param0: Bytes): ethereum.CallResult<Address> {
    let result = super.tryCall("bonds", "bonds(bytes32):(address)", [ethereum.Value.fromFixedBytes(param0)]);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
  ...
  createBond(_collateralToken: Address, trancheRatios: Array<BigInt>, maturityDate: BigInt): Address {
    let result = super.call("createBond", "createBond(address,uint256[],uint256):(address)", [
  ...
  try_createBond(
    ...
    let result = super.tryCall("createBond", "createBond(address,uint256[],uint256):(address)", [
      ethereum.Value.fromAddress(_collateralToken),
      ethereum.Value.fromUnsignedBigIntArray(trancheRatios),
      ethereum.Value.fromUnsignedBigInt(maturityDate),
    ]);
  ...
  try_trancheFactory(): ethereum.CallResult<Address> {
    let result = super.tryCall("trancheFactory", "trancheFactory():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
